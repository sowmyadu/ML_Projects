...
class linear_layer:
    ...
    def forward(self, X):
        ...
        prod = np.dot(X,self.params['W'])
        forward_x = np.add(prod,self.params['b'])
        #print("Shape is:",forward_x)
            
        forward_output = forward_x
    ...
    def backward(self, X, grad):
        ...
        N, output_D = grad.shape
        b_derivative = np.ones((1, N))
        self.gradient['b'] = np.dot(b_derivative,grad)
...
class tanh:
    ...
    def backward(self, X, grad):
        ...
        a = np.array(grad);
        b = np.array(derivative)
        backward_output = np.multiply(a,b)
        ####################################################################################################
        # TODO: Implement the backward pass
            ...
            for key, _ in module.params.items():
                ...
                if _alpha > 0.0:
                    ...
                    if key == 'W':
                        ...
                        module.params['W'] = module.params['W'] + m;
                    else:
                        m = momentum[module_name+'_'+key]
                        m = _alpha * m - _learning_rate * g
                        module.params['b'] = module.params['b'] + m;
                    ...
                    if key == 'W':
                        module.params['W'] =  module.params['W'] - _learning_rate * g
                    else:
                        module.params['b'] = module.params['b'] - _learning_rate * g
